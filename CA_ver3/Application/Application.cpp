...
//ATTENTION: 
{
	if (main_heap->idsearch(main_heap->getMin(), id) != NULL)                      //the node is in main heap
	{
		return main_heap->idsearch(main_heap->getMin(), id);
	}
	else                                                     //the node is not in main heap
	{
		if (medium_risk_heap->idsearch(medium_risk_heap->getMin(), id) != NULL)           //the node is in medium risk heap
		{
			return medium_risk_heap->idsearch(medium_risk_heap->getMin(), id);
		}
		else
		{
			if (high_risk_heap->idsearch(high_risk_heap->getMin(), id) != NULL)         //the node is in high risk heap
			{
				return high_risk_heap->idsearch(high_risk_heap->getMin(), id);
			}
			else
			{
				if (withdraw_heap->idsearch(withdraw_heap->getMin(), id) != NULL)      //the node is in withdraw heap
				{
					return withdraw_heap->idsearch(withdraw_heap->getMin(), id);
				}
