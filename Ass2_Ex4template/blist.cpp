...
template<class T> AList<T>* BList<T>::bucketsort(void)
{
	...
	a = AList<T>::getmin();
	b = AList<T>::getmax();
	int n = AList<T>::getlength();
	AList<T>* bucketlist;
	AList<T> ans;

	bucketlist = new AList<T>[n];
	for (int i = 0; i < n; i++) {
		bucketlist[i].setmin(a);
		bucketlist[i].setmax(b);
	}

	for (int i = 1; i <= n; i++)
	{

		T value = AList<T>::getitem(i);
		int index = (value - a) * n / (b - a);
		...
		if (0 == bucketlist[index].getlength()) {
			...
			for (int j = 1; j <= bucketlist[index].getlength(); ++j) {
				...
				if (value < bucketlist[index].getitem(j) || j == bucketlist[index].getlength()) {
					bucketlist[index].insert(j, value);
					break;
				}
	...
	for (int i = 1; i < n; i++) {
		bucketlist[0].concat(bucketlist[i]);
		//bucketlist[i].prettyprint();
	}
	//bucketlist[0].prettyprint();
	return &bucketlist[0];
