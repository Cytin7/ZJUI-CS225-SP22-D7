...
template<class T> void AList<T>::allocate(void)
{
    ...
    for (int i = 0; i < numitems; ++i)// Reload a new array
    {
        newarray[i] = reprarray[i];
    }
    delete[] reprarray; // don't forget to save space
    reprarray = newarray;
    maxsize = newsize; // How about the minsize?
    return;
}
...
template<class T> void MaxHeap<T>::build_heap(AList<T> &array){
    //put your code below
    AList<T>::concat(array); //First enlarge Alist based on given array
...
template<class T> void MaxHeap<T>::swap(int i, int j){
    //the index of this array is begin at 1;
    T temp = AList<T>::getitem(i);// use a temp variable to store 
